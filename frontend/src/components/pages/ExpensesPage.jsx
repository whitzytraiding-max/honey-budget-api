...
const CATEGORIES = [
  ...
  "Gifts", "Taxes", "Emergency", "Travel", "Wellness",
];

function TransactionCard({ transaction, currentUserId, onEdit, onDelete, t }) {
  return (
    <div className="hb-panel-soft rounded-[1.35rem] border border-sky-100 p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate font-semibold text-slate-900">{transaction.description}</p>
          <p className="mt-1 text-xs uppercase tracking-[0.18em] text-slate-500">{transaction.date}</p>
        </div>
...
function ExpensesPage({
  ...
  const mmkRateText = mmkRateData?.rate
    ? `1 USD = ${Number(mmkRateData.rate.rate).toFixed(2)} MMK · ${String(mmkRateData.rate.rateSource ?? "custom").toUpperCase()} · ${String(mmkRateData.month).padStart(2, "0")}/${mmkRateData.year}`
    : "";
      ...
      <section className="hb-surface-card rounded-[1.75rem] p-4 sm:p-6">
        ...
        <div className="space-y-3">
          {recentTransactions.length ? (
            ...
            <div className="hb-panel-soft rounded-[1.35rem] border border-sky-100 px-4 py-8 text-center text-sm text-slate-500">
              {t("expenses.empty")}
            </div>
          )}
        </div>
  ...
  if (activeTab === "history") {
    return (
      <section className="hb-surface-card rounded-[1.75rem] p-4 sm:p-6">
        ...
        <div className="hidden overflow-hidden rounded-[1.5rem] border border-sky-100 sm:block">
            ...
            <table className="min-w-full divide-y divide-slate-100 text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 bg-white/92">
                {transactions.length ? (
                  transactions.map((transaction) => (
                    <tr key={transaction.id}>
                      <td className="px-4 py-4">
                        ...
                        <p className="text-xs uppercase tracking-[0.18em] text-slate-500">{transaction.date}</p>
                      </td>
                      <td className="px-4 py-4 text-slate-600">{transaction.category}</td>
                      <td className="px-4 py-4 text-slate-600">{transaction.paymentMethod === "cash" ? "Cash" : "Card"}</td>
                  ...
                  <tr>
                    <td className="px-4 py-8 text-slate-500" colSpan="5">{t("expenses.empty")}</td>
                  </tr>
                )}
              </tbody>
            </table>
    ...
    <section className="hb-surface-card rounded-[1.75rem] p-4 sm:p-6">
      ...
      <form className="mt-4 grid gap-3 sm:mt-6 sm:gap-4" onSubmit={onExpenseSubmit}>
        <div className="grid gap-4 sm:grid-cols-2">
          <Input label={t("expenses.amount")} name="amount" type="number" min="0" step="0.01" required value={expenseForm.amount} onChange={onExpenseChange} placeholder="86.50" />
          <Input label={t("expenses.description")} name="description" value={expenseForm.description} onChange={onExpenseChange} placeholder={t("expenses.dateNightTacos")} />
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <Select label={t("expenses.category")} name="category" value={expenseForm.category} onChange={onExpenseChange} options={CATEGORIES.map((entry) => ({ label: entry, value: entry }))} />
          <Select label={t("expenses.entryCurrencyLabel")} name="currencyCode" value={expenseForm.currencyCode} onChange={onExpenseChange} options={currencyOptions} />
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <Input label={t("expenses.date")} name="date" type="date" required value={expenseForm.date} onChange={onExpenseChange} />
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <ToggleGroup label={t("expenses.type")} name="type" value={expenseForm.type} onChange={onExpenseChange} options={[{ label: t("expenses.oneTime"), value: "one-time" }, { label: t("expenses.recurring"), value: "recurring" }]} />
          <ToggleGroup label={t("expenses.method")} name="paymentMethod" value={expenseForm.paymentMethod} onChange={onExpenseChange} options={[{ label: "Card", value: "card" }, { label: "Cash", value: "cash" }]} />
        </div>
        ...
        {!editingTransactionId && householdUsers?.length >= 2 && (() => {
          ...
          return (
            <div>
              <p className="mb-1.5 text-xs font-semibold uppercase tracking-[0.15em] text-slate-500">Log expense under</p>
              ...
              {activeId === "joint" && (
                <p className="mt-1.5 text-xs text-slate-500">The total will be split equally — half logged under each of you.</p>
              )}
            </div>
          );
        })()}
